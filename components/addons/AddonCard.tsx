...
export default function AddonCard({ addon, userId, status }:{
  ...
  async function purchase() {
    ...
    setLoading(false);
    if(!r.ok) return alert('Error'); location.reload();
  }
  ...
  return (
    <motion.div initial={{opacity:0,y:12}} whileInView={{opacity:1,y:0}} transition={{duration:.35}}
      ...
      <div className="mt-5">
        {owned ? (
          <span className="inline-flex px-3 py-1 rounded-lg bg-emerald-100 text-emerald-700 text-sm">Active</span>
        ) : (
          <button disabled={loading} onClick={purchase}
            className="px-4 py-2 rounded-lg text-white bg-gradient-to-r from-sky-500 to-blue-700">
