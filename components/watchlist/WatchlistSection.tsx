...
function save(items: WLItem[]) {
  try { localStorage.setItem(STORAGE_KEY, JSON.stringify(items)); } catch {}
}

function priority(v: Verdict) {
  if (v === "green") return 1;
...
export default function WatchlistSection() {
  ...
  async function runCheck(it: WLItem) {
    ...
    // notify if threshold matched or exceeded
    ...
    <section className="mt-12 rounded-2xl border border-slate-800 bg-slate-900/40 p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs text-slate-400">
          Storage: <strong>localStorage</strong> (persists until you clear site data)
        </span>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-2">
        ...
        <input
          ...
          placeholder={type === "Wallet" ? "rXXXXXXXXXXXXXXXXXXXXXXXX" : "example.com"}
          className="flex-1 px-3 py-2 rounded-lg bg-slate-800 text-slate-200"
        />
        ...
        <button
          onClick={addItem}
          className="px-4 py-2 rounded-lg bg-cyan-700 hover:bg-cyan-600 text-white font-medium"
        >
          Add
        </button>
      </div>
      ...
      <input
        ...
        className="w-full mt-2 px-3 py-2 rounded-lg bg-slate-800 text-slate-300"
      />
        ...
        <table className="min-w-full text-sm">
          <thead className="text-slate-400">
            <tr>
              ...
              <th className="text-left py-2 pr-4">Last verdict</th>
              ...
              <tr key={it.id} className="border-t border-slate-800">
                ...
                <td className="py-2 pr-4">
                  {it.lastVerdict ? (
                    <span className={`px-2 py-1 rounded text-xs ${
                      it.lastVerdict === "green" ? "bg-emerald-900/40 text-emerald-300" :
                      it.lastVerdict === "orange" ? "bg-amber-900/40 text-amber-300" :
                ...
                <td className="py-2 pr-4 flex gap-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1 rounded bg-red-900/60 hover:bg-red-800/70"
                  >
                    Remove
                  </button>
                </td>
