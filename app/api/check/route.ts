...
async function getAccountAgeDays(address: string): Promise<number | null> {
  try {
    ...
    const fallbackLedger = await rpcWithFallback<LedgerResult>(ledgerBody);
    if (fallbackLedger.error || !fallbackLedger.data) return null;
    const ledgerData = fallbackLedger.data;
